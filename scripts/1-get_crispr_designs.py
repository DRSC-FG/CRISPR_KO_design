...
#
# exons all combine to create CDS
# CDS is region that protien codes between the start and stop codons.
# transcriptome is the whole sequence given here.
#
    ...
    for line in f:
        ...
        if 'start_codon' in feature or 'stop_codon' in feature or 'transcript' in feature or 'CDS' in feature or 'exon' in feature:
            ...
            if not (transcripts[gene][transcript]['CDS']):
                transcripts[gene][transcript]['CDS'] = []
            ...
            if not (transcripts[gene][transcript]['exon']):
                transcripts[gene][transcript]['exon'] = []
...
def dump_gtf_summary(gtf, fasta):
    print ("+Writing Debug file " +data_dir + '/output/base_cds_and_exon_info.txt')
    f = open(data_dir +  '/output/base_cds_and_exon_info.txt', 'w')

    mykeys = ['start_codon','lowest_CDS', 'highest_CDS','CDS+', 'lowest_exon', 'highest_exon','exon+']

    for onekey in mykeys:
        f.write(onekey+'\t')
    f.write('\n')
    ...
    for record in SeqIO.parse(fasta, "fasta"):
        ...
        if gtf[gene][seqid]:
            ...
            #set unknown features to NA
            for onekey in mykeys:
                if onekey not in features:
                    features[onekey]='0'
            ...
            for onekey in mykeys:
                if onekey=='CDS+':
                    delta = int(features['highest_CDS'])- int(features['lowest_CDS'])
                    f.write(str(delta)+'\t')
                elif onekey=='exon+':
                    delta = int(features['highest_exon'])- int(features['lowest_exon'])
                    f.write(str(delta)+'\t')
                else:
                    f.write(str(features[onekey])+'\t')
            f.write('\n')
...
dump_gtf_summary(gtf,  data_dir + '/input/transcripts.fa')


print ("+Reading: "+data_dir + '/input/transcripts.fa')
